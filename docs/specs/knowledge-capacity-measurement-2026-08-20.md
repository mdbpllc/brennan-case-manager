...
- **`Q-CAP-2`** — **Does `/docs/reference/` leave the sync picker?** §4.1. His hand, no code, ~3.9%.
  ***[RULED IN by Michael 2026-08-20, the day this document was filed — his words:*** “rule Q-CAP-2 in — drop /docs/reference/ from the picker.” ***The RULING is recorded; the ACT is his hand in the Claude project UI and no code push can perform it, so the exclusion is not in effect until he clicks it. And the ruling is properly a RE-AFFIRMATION: queue row `WS-P1` already recorded this exclusion as RULED on 2026-08-12 and open only because the live config showed no such exclusion — a prior ruling this document's §4.1 and §5 did not cite. `WS-P1` is annotated and stays OPEN, because what it awaits is the click. No figure in this document was refreshed, recomputed or altered; nothing was deleted — excluding from sync is not deleting from the repo, and `CR3-code-sheet-2023.pdf` is untouched. `Q-CAP-1`, `Q-CAP-3`, `Q-CAP-4` and `Q-CAP-5` are UNTOUCHED and still open, and `Q-CAP-1`/`Q-CAP-3` are still paired.]***
