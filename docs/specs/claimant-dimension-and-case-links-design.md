...
| **D-CL2-3** | Fee arrangement per client — does the time tracker's "one rate per case, uniform" rule survive, or become one rate per client? | **CLOSED (Michael, 2026-08-07): ONE RATE PER CLIENT.** The rate moves off the case onto the client record, beside `fee_arrangement`; single-client cases render exactly as today (D-CL2-7). Claude's keep-it-case-level recommendation was **withdrawn on Michael's facts, not deference**: *"I will have some cases where I have two clients and one will have a discounted rate for a certain reason and the other client will have the standard rate."* **Mid-case changes are PROSPECTIVE**, via an effective-dated **rate history** on the client record: *"the hours already billed would likely stay the same because we already contracted for this and I would discount the hours incurred going forward."* Every hour is valued at the rate in force when incurred — including unbilled-but-already-incurred hours, which keep the contracted rate. **Nothing silently revalues** (the `toRow` silent-mutation lesson applied to money); a retroactive courtesy is a **visible write-down at invoicing**, never a recompute. Rejected alternatives, each encoding a rule: current-rate-rules (unbilled history would silently reprice) and snapshot-plus-bulk-tool (unnecessary once history is effective-dated). Full reasoning: `rulings-capture-2026-08-07.md` Part 5. **Authorizes nothing** — the time tracker stays parked behind CE1, which stays unauthorized |
| **D-CL2-3a** | **Fee-affidavit export under per-client rates** — the export (time-tracker doc §5) assumed one uniform case rate because that is lodestar-shaped. **Which rate does the affidavit carry when clients differ?** | **OPEN — new 2026-08-07**, a noted consequence of D-CL2-3, not a decision. Claude's lean — the rate of the client whose claim carries the fee demand — is **PROPOSED, unruled**. Michael's **→ RULED 2026-08-13 (#66): the affidavit carries the rate of the CLIENT WHOSE CLAIM CARRIES THE FEE DEMAND; multi-client demands get PER-CLIENT EXHIBITS. Authorizes nothing — the tracker stays parked behind CE1. Annotated 2026-08-13 (#68).** |
| **CL2-AC-1** | **Auto-create the client record when a party is linked with the Client role on a PI case** — a PI client with no damages-scope record is an impossible real-world state. Data model stays parallel (D-CL2-8); UI workflow only. Brushes directly against deferred CL2-CHECK-1 — "auto-create on PI" and "flag Client-role-without-record" are two answers to the same gap | **DIRECTION-CONFIRMED 2026-08-08 — auto-create is policy; UNRULED edges: link-removal, mixed-posture, backfill (doc-on-screen). See `rulings-capture-2026-08-08b.md` Part 2 and the queue entry.** ID issued 2026-08-07 for the 2026-07-28 walkthrough capture, so the question cannot die the K-6/K-7 death. Fuller framing in `attorney-review-queue.md` §7 |
