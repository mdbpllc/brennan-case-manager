...
| **D-CL2-3a** | **Fee-affidavit export under per-client rates** — the export (time-tracker doc §5) assumed one uniform case rate because that is lodestar-shaped. **Which rate does the affidavit carry when clients differ?** | **OPEN — new 2026-08-07**, a noted consequence of D-CL2-3, not a decision. Claude's lean — the rate of the client whose claim carries the fee demand — is **PROPOSED, unruled**. Michael's |
