...
Stand up one vertical slice end-to-end (Case overview + Parties). STARTED — v0.1 BUILT AND DELIVERED (2026-07-21). Working React + TypeScript app: case list/create/detail with auto YY-NNNN file numbers, cascading practice-area→case-type classification, PI flags, per-type status lists; full party directory with a config-driven party-type registry covering every §11 party type (registry-rendered forms incl. repeating groups and party-to-party links); case↔party linking with roles/sides; cross-case history on every party. Architecture: data-adapter layer — localStorage demo mode (zero-setup, seeded fictional data) + Supabase adapter and full Postgres schema (db/schema.sql, server-side gapless file numbers, RLS placeholders) ready to activate via .env. Delivered as brennan-case-manager-v0.1.zip in the 2026-07-21 chat; README inside has Michael's setup steps (Node LTS → npm install → npm run dev). NEXT: Michael runs it and gives feedback; then the second slice below.
...
Vertical slice v0.1 — BUILT AND DELIVERED (2026-07-21). See section 5, item 2. AWAITING: Michael to run it (README setup) and give feedback. Demo mode holds no real client data; Supabase hookup + auth + security review come before any real data.
