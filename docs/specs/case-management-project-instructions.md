...
**A mis-parented `Probate companion` case type already exists in source** (`src/domain/caseTypes.ts`), sitting **under Personal Injury** and inheriting the `_piDefault` status ladder, which the code's own comment concedes does not fit its arc. S-1 rules probate a practice line **in its own right**, which that structure contradicts. **Re-parenting it — and fixing the ladder — is gated on PR-3 (Michael's), and converges with the pending probate-ladder design pass already logged as `docs/spec-feedback.md` item 1.** Nothing is re-parented, no ladder is changed, and nothing is built until he rules.
...
Local AI-processing arm (NEW): the transcript layer's transcription/diarization runs LOCALLY on Michael's NVIDIA-GPU PC (Canary via NeMo) — see docs/specs/transcript-workflows.md §§8–9. This is a deliberate architecture decision (privilege/PHI posture), not a hosting shortcut. The same local-first posture governs AI processing of medical bills/EOBs (PHI) in the billing module — its Phase 1b ingestion waits on this arm.
...
Transcript design questions — COMPLETE (2026-07-21). All six answered; decisions integrated into docs/specs/transcript-workflows.md §8 and summarized at feature 14 above.
Plea-hearing eligibility reminder — COMPLETE (2026-07-21). Fully specified from the project statutes file; spec at docs/specs/plea-hearing-eligibility-reminder.md. Two defaults chosen without asking (adjustable): 3-day reminder lead time; fires at plea-negotiation stage entry as well as calendared settings.
...
Citizens MRF dry run — COMPLETE (2026-07-22), including the per-code comparison. The 55 MB live file (CMS v3.0.0, dated 2026-05-11) was staged from Michael's Downloads via the desktop bridge and analyzed; findings + Phase 2 loader requirements in docs/specs/citizens-mrf-dry-run.md. The file is the Phase 2 reference fixture (Michael retains the original in Downloads; re-stage or re-download at build time).
