...
| **§14.1 / §14.6** | `AS-Q4` — extracted TEXT per version in the database, bytes not retained, a file store at gate 7; `AS-Q10` — one chronology set per CLIENT; **AMENDED 2026-09-09 (`#154`), one limb — see the annotation beneath §14.1** | **2026-08-31 late** |
...
> **AMENDED 2026-09-09 (`#154`), ONE LIMB — the storage location of the extracted text. RULED by Michael at the API-integrations ruling sitting (`docs/specs/api-integrations-ruling-sheet-2026-09-09.md` §1 items 3–6, §2).** The selection above stands in every limb but one: the extracted text per version is now **a text file in the matter's OneDrive folder (per client, as `AS-Q10` has it)**, and the `case_chronology_versions` row keeps **version number, drive item id, content hash, `readable`, `char_count`, `removed_at` — and no text**. Three forks ruled with it: **the original dropped file is KEPT beside the text file** in that folder (point-don't-copy when it already lives in the CloudLex mirror) — so "bytes not retained" now reads *"bytes not retained by the app's database"*; **removal (D-60) MOVES both files to a firm-level `_removed` folder outside any matter**, pointer kept, deletion Michael's hand; **the browser writes and extracts now** (delegated `Files.ReadWrite`, ruled IN, consent his act at build), **the P1 service later** when T3 is authorized. What the model receives, the unreadable-scan flag, one chronology set per client, and the file store at gate 7 are UNCHANGED. **The built table's `extracted_text` column is a spec-vs-code gap until a later, separately authorized slice moves it** (`docs/spec-feedback.md`, the 2026-09-09 section; `Q-API-20`). Reason for the amendment, recorded because it is load-bearing: the extracted text and the five disclosures tables are PHI on the record's own reading, and keeping PHI in Supabase costs its Team plan (from $599/month) — decision 1 of the same sitting keeps PHI inside the Microsoft tenant.
...
**ALL FIVE ARE CLOSED — RULED 2026-09-05 at the CC-1 hands-on sitting (`#148`; `docs/specs/cc1-hands-on-sitting-rulings-2026-09-05.md` §1, R1–R4, Q5 and Q10 ruled together as R3).** With the four accepted register rows closed the same day (`CL2-AC-1`, `CL2-CHECK-1`, `FE-§11.4`, the bill-label pre-fill), **nine of the twenty-one accepted items close, so TWELVE accepted items REMAIN OPEN on the CC-1 queue — every one of them GATED on an unbuilt module, each named in the rulings doc §7.** *(This does not disturb the ⚠ line below: twenty-one is and stays the number ACCEPTED; twelve is how many of them are still open.)*

| Held item | The question | Build default while held |
|---|---|---|
| **the stop's SHAPE** (§12.3) | When a must-fix condition fires — no incident date, no facility name, no facility type — is the stop a must-fix tier inside the `HD-1` panel, or a dialog on the generate button? | none needed to build the rule; the surface is his. **RULED 2026-09-05 — `#148`; see `docs/specs/cc1-hands-on-sitting-rulings-2026-09-05.md` §1 R2. No default was held — the surface is now RULED: the top-of-page "Fix these first — N" card stays, the Generate button is disabled and STATES the count, no dialog, not merged into the `HD-1` panel.** |
| **ND-7(c)** (§4) | A provider on the charges table has no designation paragraph: warn (panel tier) or stop (must-fix tier)? Michael: *"Im not sure when this will become an issue. There is always a provider with a facility."* | a PANEL LINE, never a stop — PROVISIONAL. **RULED 2026-09-05 — `#148`; see `docs/specs/cc1-hands-on-sitting-rulings-2026-09-05.md` §1 R1. The default is SUPERSEDED FOR TIER 2 ONLY — a billed `providerBusiness` with no chronology row is a MUST-FIX; tiers 1 and 3 keep the panel line.** |
| **Q5** (§8) | The gap flag's trigger — absolute dollar, percentage of the damages total, rank, or attorney-set? | none; the flag's surface (the ambient line) is settled. **RULED 2026-09-05 — `#148`; see `docs/specs/cc1-hands-on-sitting-rulings-2026-09-05.md` §1 R3. No default was held — ruled with Q10 as one question: the gap line carries the facility's billed total inline and gap lines sort charge-descending. NO THRESHOLD; panel tier.** |
| **Q10** (§8) | The custodian-degradation threshold (FE-22). | none; the automatic limb and the persisted gap flag survive. **RULED 2026-09-05 — `#148`; see `docs/specs/cc1-hands-on-sitting-rulings-2026-09-05.md` §1 R3. No default was held — ruled with Q5 as one question; the FE-22 "threshold" RETIRES AS A CONCEPT.** |
| **the rider's supervisor** (§15.6) | Beneath a COLLECTIVE paragraph, what does the fixed scope sentence's `{supervising_provider}` name — the group, or a supervisor Michael designates by hand? | GROUP FILL — PROVISIONAL. **RULED 2026-09-05 — `#148`; see `docs/specs/cc1-hands-on-sitting-rulings-2026-09-05.md` §1 R4. The default STANDS AS THE RULE — GROUP FILL always, no longer provisional; no hand-designated supervisor and no per-rider override.** |
