...
> **POINTER, ADDED 2026-08-31 (late sitting) — NOT A FOLD-IN.** Eighteen further rulings were taken on 2026-08-31 in a second typed sitting (Fable 5, Cowork), each a selection by Michael among option texts put with a recommendation (`AS-Q1`–`AS-Q13`): the model call's server-side shape and fixture-only posture; the extraction contract (names, credentials, dates, visit rows, per-individual summaries — never type, role or facility membership); the provider record's CASE-SCOPED home with promote-to-directory by hand; text-per-version chronology storage; three added types (mental health; other physician (MD/DO); other licensed non-physician provider); supplementation's base and scope; the custodian shapes (§9.11 placed whole by the app; four limbs for the pharmacy; the block's line strings); the short-form causation line for the six medical-probability types, the bold name lead and the rider's fixed sentence; the four interview cards retired for the treating track; one instrument per plaintiff; the rename's literals and one migration file gated on `MIG-1`; five docs acts; scope and the authorization's form. **They live at `docs/specs/fe-d1-amendment-slice.md` §2 (his selections quoted) and in the session log; §§11–§18 of this file do not yet carry them — the THIRD FOLD-IN is owed and is an Opus design session's act.** Where §§11–§18 and the slice's §2 differ, the later ruling governs. In particular §17.1a's phrase "the model populates NAMES and nothing else" is read, by `AS-Q2`, as barring type, role and facility membership only.
