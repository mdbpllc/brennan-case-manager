...
Commit: 5ed8d59  |  Branch: master  |  Generated: 2026-08-13 Central (sixtieth refresh)
...
- **Benchmarks import reads `..\data\pfs\` — NOT present on the machine that ran this refresh** (re-checked; it lives on the other machine). **Outlook push WORKS as of 2026-07-26 but ONLY event CREATION is exercised** — edit/cancel unverified; demo events only
...
- **REGISTRY — THREE files.** **TWENTY VERIFIED** (Michael) in `legal-rule-registry-discovery-and-carrier-duties.md`; **three wording expansions there are FLAGGED, NOT ADOPTED** (TRCP 194's span, 193.3, 192.3(f)) — verification attaches to wording. **TWENTY-SEVEN UNVERIFIED** in `legal-rule-registry-discovery-enforcement-and-pleading.md` (**six WL/slip cites flagged, none corrected**). **SEVEN UNVERIFIED** in `legal-rule-registry-criminal-plea-and-costs.md` — six as filed, then **former entry 4 SPLIT by your ruling 08-13 into 4a** (separate criminal actions: per-cause costs, still CITE-LESS) **and 4b** (single criminal action: each cost once at the highest category, art. 102.073). **BACKLOG 34.** Entry 3 and 4a carry NO CITE — supplying one is YOUR act. **CR-10's cost check must ask same-action-or-separate BEFORE totaling.** **`/rules` SEED ALL UNVERIFIED.** Registry files were deliberately SCOPED OUT of the drift sweep — no wording touched
...
- **ALSO YOURS: paste the revised v17** · **re-upload SKILL.md v2** (SK-v2) · **the sync-picker clicks** — `docs/skills/` (Q-3), `docs/templates/`, and the `docs/reference/` exclusion · **telemetry lockdown on the P1** (confirmed NOT set) · **HK-6: locate the T3 kickoff-day capture** (adoption RULED; execution gated on you supplying the file) · **V-2, the cite-check one-look list, Ochoa first** · **OBS-1** · **a fresh meter reading — requested at #68 and not stated; both 08-13 sessions were heavy Fable**
...
- **RUNNER DISCIPLINE — QR-1..QR-4 + MM-1 CLOSED; runner v7** (`docs/prompts/QUEUE-RUNNER.md`, the **ONLY full copy**). **Your hand: the P15 user-level copy deletion** (P1 ABSENT). **The 08-13 gap is CLOSED in the text (#67): Step 0 now STOPS when HEAD is AHEAD of origin/master** — previously it stopped only on behind/dirty/diverged/off-master, so the one state meaning "the design side hasn't seen these commits" had no guard. **The v7 gate was exercised and passed on its own terms this batch**
- **THE H-STRING COLLISION IS ENDED:** the log-#44 housekeeping series re-lettered **H → HK** (HK-1, HK-2, HK-4, HK-5, HK-6). **Heartbeat H1–H83 untouched — never rename them.** The naming caveat covers **IN / DE / CR / HK**
...
- **Everything awaiting your ruling is in `docs/specs/attorney-review-queue.md`** — reconciled
  through **#68**. **K-6/K-7 RETIRED — reconstruct NOTHING.** **UNRULED, adopt nothing:
