...
Commit: 1209a40  |  Branch: master  |  Generated: 2026-08-13 Central (fifty-eighth refresh)
...
- **v17 IS DRAFTED, DELIVERED, AND NOT YET PASTED — YOUR HAND.** It carries Descrybe-out, SAT-1 (adopted both parts), the §7.2 model-routing clause with refreshed economics (closing **Q-5** and **HK-2**), the **HK-5** rule, and the two touch-ups. **Until you paste it, none of that is in force for design sessions.** v16 is what a session reads today; DT-1 remains in force. The next fresh design session verifies v17 (#54 precedent)
...
- **RUNNER DISCIPLINE — QR-1..QR-4 + MM-1 CLOSED; runner v6** (`docs/prompts/QUEUE-RUNNER.md`, the **ONLY full copy**). **Your hand: the P15 user-level copy deletion** (P1 ABSENT). **NEW FLAG 08-13: the QR-3 gate does NOT catch an unpushed tree** — it stops on behind/dirty/diverged/not-on-master, and *ahead* is none of those, so a packet cannot rely on it to enforce push-ordering
