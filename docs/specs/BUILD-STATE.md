...
- **REGISTRY RESEARCH SUPPORT — THREE DOCS, ALL PROPOSED, FLAGS-ONLY, NOTHING VERIFIED, NO REGISTRY
  ...
  Collins needs retrieval) · #72: `registry-verification-workbook-2026-08-13.md` — **all 34 entries, one row each, each reduced to ONE look** — and `registry-citator-pass-2026-08-13.md` (majority resolution + citing picture).
...
- **V-4–V-8 ALL RULED 08-13 (#73) — AND NONE OF IT IS EXECUTED. No registry file, no design doc, no CLAUDE.md was touched; specs are read-only here and every one is a wording act.** V-4 keep as two, cross-referenced (consolidating merges across a status line and drops three banked verifications); V-5 split all three two-case entries — **BACKLOG BECOMES 37 WHEN EXECUTED; it is 34 in the files today**; V-6 both criminal entries stay, reworded (removing them would NOT unblock CR-10); V-7 narrow entry 23; V-8 record all three hazards
...
- **CHAT-DISPATCH: Tasks 1+2 COMPLETE (#72, the atomic pair) and TASK 3 COMPLETE (#74); Tasks 4–19
  NOT STARTED — resume at Task 4.** **The bridge DID connect for Task 3** and all 96 synced files
  were read full-text at `aa033dc` — the first packet in the chain sourced to HEAD, not RAG. **But
  HK-7 STAYS OPEN: the grant is session-scoped, satisfied for one session, NOT closed**, and Tasks
  5, 17, 18 still need it re-granted. **FLP throttles at 5 req/min** — budget it
- **CD-2 ROLE MINING PASS FILED (#74) — `docs/specs/cd2-role-mining-pass-2026-08-13.md`, 350 non-blank lines, PROPOSED design input, DATA PREP ONLY. Nothing built, nothing ruled, no spec touched;** `contact-directory.md` §4/§5/§6 are unchanged and the pass proposes *to* them. **Its reframing finding: CD-2 is NOT an open design question** — framing ruled #48, both layers ruled #51, structure built #61 — so it is a **coverage audit of a ruled taxonomy**, every row marked BUILT / RULED-NOT-BUILT / CANDIDATE / DECLINE. **The honest gap: the authoritative role-tag vocabulary in `src/domain/partyRegistry.ts` and `src/domain/roster.ts` was NOT read**, so ~60 rows carry `TAG-CHECK: NOT RUN`. **Ten questions became CD-4 – CD-13** (collision-checked; the series was at CD-3). Headline three: **CD-4** paste the two files or route the comparison to Code; **CD-5** are roles defined by the ABSENCE of another role expressible at all; **CD-6** should `case_parties.role` stay free text when the other five ruled axes are all DB-enforced
...
- **ALSO YOURS: paste the revised v17** · **re-upload SKILL.md v2** (SK-v2) · **the sync-picker clicks** — `docs/skills/` (Q-3), `docs/templates/`, and the `docs/reference/` exclusion · **telemetry lockdown on the P1** (confirmed NOT set) · **HK-6: locate the T3 kickoff-day capture** · **HK-7: re-grant the checkout each session** · **V-2, the one-look list, Ochoa first** · **V-9, the binding-rule gap** · **OBS-1** · **CD-4 – CD-13, ten role-model questions; CD-4 is the cheap one** (two files pasted, or one word routing the comparison to Code, converts ~60 uncertain rows) · **a fresh meter reading — requested at #68 and still not stated**. **V-4–V-8 are OFF this list — ruled 08-13; they need EXECUTION design-side, not a ruling**
...
- **RUNNER DISCIPLINE — QR-1..QR-4 + MM-1 CLOSED; runner v7** (`docs/prompts/QUEUE-RUNNER.md`, the **ONLY full copy**). **Your hand: the P15 user-level copy deletion** (P1 ABSENT). **Step 0 STOPS when HEAD is AHEAD of origin/master** (#67) — **the v7 gate has now passed on its own terms three times**, at #68, #72, and #74. **NEW CONSTRAINT (#74): QR-3 CANNOT RUN THROUGH THE DEVICE BRIDGE and must stay native** — through the bridge mount `git status` reads 199 modified files (37,271 insertions / 37,271 deletions) while `git diff --ignore-cr-at-eol` is EMPTY (pure CRLF), so the gate would stop on a **false DIRTY**; and `git fetch` fails there (`HTTP 403 from proxy`, no network on the device VM), so a local `HEAD == origin/master` read is an **unfetched ref, not a pass**
...
- **Everything awaiting your ruling is in `docs/specs/attorney-review-queue.md`** — reconciled
  through **#74**. **K-6/K-7 RETIRED — reconstruct NOTHING.** **UNRULED, adopt nothing:
