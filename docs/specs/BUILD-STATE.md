...
Commit: 24429d9  |  Branch: master  |  Generated: 2026-08-13 Central (fifty-ninth refresh)
*Line counts in this record are NON-BLANK lines (ruled 08-13, #67); raw counts are labeled "raw".
This file's 150-line cap is a non-blank cap — which is how it has always been measured.*
...
- **TELEMETRY LOCKDOWN IS NOT IN PLACE — CONFIRMED NOT SET 08-13 (#66).** Michael ran the self-check; **both machine-scope variables came back EMPTY.** Recipe re-supplied with the WSL2 half emphasized: `docs/gpu-telemetry-offline.md`. **Still his hand, on the GPU machine; closes only when he reports verification 3 passed. No privileged audio before then**
...
- **No case-event/CE table, no time_entries, no claims, no `case_links`. Nothing exists for the DE
  series** — no response-set, response-item, or escalation-timeline table. Health **re-measured this
  refresh: 278 tests pass (23 files), build + lint clean**
...
- Time tracker: draft only. Servpro deadline engine: DESIGN ONLY. Heartbeat: design docs only
  (captures e+f NOT folded; register H1–H83; **the four 08-13 rulings H40/H50/H60/H77 live in the
  queue + log and are NOT folded into the design docs yet** — that rides the heartbeat pass).
  **CE1 still NOT AUTHORIZED** (D-CL2-9) — must be **CLIENT-AWARE from the start**. CourtListener:
  design doc only, integration UNAUTHORIZED (Q-6). **Descrybe is OUT by ruling 08-13**
- **REGISTRY — THREE files now.** **TWENTY VERIFIED** (Michael) in `legal-rule-registry-discovery-and-carrier-duties.md`; five took **second observations** 08-12, and **three wording expansions there are FLAGGED, NOT ADOPTED** (TRCP 194's span, 193.3, 192.3(f)) — verification attaches to wording. **TWENTY-SEVEN UNVERIFIED** in `legal-rule-registry-discovery-enforcement-and-pleading.md` (discovery enforcement + UDJA/venue/pleading; **six WL/slip cites flagged, none corrected**). **SEVEN UNVERIFIED** in `legal-rule-registry-criminal-plea-and-costs.md` — six as filed, then **former entry 4 SPLIT by your ruling 08-13 into 4a** (separate criminal actions: per-cause costs, still CITE-LESS) **and 4b** (single criminal action: each cost once at the highest category, art. 102.073). **BACKLOG 33 → 34.** Entry 3 and 4a still carry NO CITE — supplying one is YOUR act. **CR-10's cost check must ask same-action-or-separate BEFORE totaling.** **`/rules` SEED ALL UNVERIFIED**
- **`docs/specs/registry-cite-check-2026-08-13.md` — the cite-check memo, PROPOSED, research support only.** Four of the six slip cites are memorandum opinions whose WL cite is PERMANENT; **Ochoa is the one where a reporter cite plausibly EXISTS ("(PUBLISH)")**; Collins needs retrieval. **NOTHING verified.** Queued as **V-2**
...
- **v17 IS DRAFTED, DELIVERED, REVISED, AND NOT YET PASTED — YOUR HAND. PASTE THE REVISED FILE (08-13 evening); the earlier same-day download is SUPERSEDED.** It carries Descrybe-out, SAT-1 (adopted both parts), the §7.2 model-routing clause with refreshed economics (closing **Q-5** and **HK-2**), the **HK-5** rule, the two touch-ups, and — folded in by the revision — the **QR-3 v7 ahead-stop** and the **line-count convention**. **Until you paste it, none of that is in force for design sessions.** v16 is what a session reads today; DT-1 remains in force. The next fresh design session verifies v17 (#54 precedent)
- **PRACTICE-PROJECT CHANNEL IS THREE: CIVIL LIT + CRIM DEFENSE + PROBATE — and both hand-items CLOSED 08-13 (V-3 CONFIRMED):** PROBATE created with v1 + carry snippets placed; CRIM DEFENSE carries v1 + the Uvalde worksheet instructions. New probate matters are CHATS inside PROBATE; the two grandfathered matter workspaces reach the build side by ONE path: **PROBATE's client-clean REQ-CAPTUREs**
...
- **CR SERIES CR-1–CR-11** (criminal-side requirements) plus a **binding black-and-white print constraint** governing all worksheet rendering. **CR-10 is HARD-GATED** on the criminal registry entries. **COLLISION FLAGGED, NOT RENAMED:** `CR-3` also means the Texas crash-report form at IN-2 — cite as "CR-3 (criminal requirement)" vs "the CR-3 crash report"
...
- **ALSO YOURS: paste v17** · **re-upload SKILL.md v2** (SK-v2) · **the sync-picker clicks** — `docs/skills/` (Q-3), `docs/templates/`, and the `docs/reference/` exclusion · **telemetry lockdown on the P1** (confirmed NOT set) · **HK-6: locate the T3 kickoff-day capture** (adoption RULED; the file is absent from project knowledge, so execution is gated on you supplying it) · **V-2, the cite-check one-look list, Ochoa first**
...
- **RUNNER DISCIPLINE — QR-1..QR-4 + MM-1 CLOSED; runner now v7** (`docs/prompts/QUEUE-RUNNER.md`, the **ONLY full copy**). **Your hand: the P15 user-level copy deletion** (P1 ABSENT). **THE 08-13 GAP IS CLOSED IN THE TEXT (#67): the Step 0 gate now STOPS when HEAD is AHEAD of origin/master** — it previously stopped only on behind/dirty/diverged/off-master, so the one state meaning "the design side hasn't seen these commits" had no guard, and a packet was relying on it. A stranded push is never silently built on
- **ONE PLACE THE NEW LINE-COUNT CONVENTION COULD STILL DRIFT, flagged not fixed: `CLAUDE.md` states the 150-line cap but does not say it counts non-blank lines.** The convention lives here, in the log, and in the unpasted v17. **Design's call whether that sentence should say it too** — Code did not edit it
- **THE H-STRING COLLISION IS ENDED (08-13):** the log-#44 housekeeping series re-lettered **H → HK**, numerals preserved (HK-1, HK-2, HK-4, HK-5, HK-6). **Heartbeat H1–H83 untouched — never rename them.** The append-only log keeps old IDs in old entries. **SAT-1 CLOSED (adopted); the two sweeps stay open.** The naming caveat now covers **IN / DE / CR / HK**
...
- **VERIFICATION STATUS: the #62/#63 and #64 batches were VERIFIED design-side FULL-TEXT on 08-13 (#65)**, read from a connected read-only checkout. Still UNREVIEWED: the carried **#31–#33** material and **#37–#61's own routing** — do not copy #36's clearance forward. **#58 was VERIFIED post-Sync, line by line**
- **Everything awaiting your ruling is in `docs/specs/attorney-review-queue.md`** — reconciled
  through **#66**, whose ruling run closed or annotated ~20 rows in one pass. **K-6/K-7 RETIRED —
  reconstruct NOTHING.** **UNRULED, adopt nothing: `future-modules-capture-2026-07-28.md`**
  (`model-routing-plan.md`'s §7.2 clause is now adopted via v17; the rest of that memo stays
  unruled). Client model: design doc §10. FOLD PENDING: captures e+f into
  case-heartbeat-design.md §8. Carried: no law-change ledger, Outlook unreviewed
