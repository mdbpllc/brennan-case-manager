...
Commit: 089715c  |  Branch: master  |  Generated: 2026-08-13 Central (fifty-seventh refresh)
...
- **No case-event/CE table, no time_entries, no claims, no `case_links`. Nothing exists for the DE
  ...
  refresh by running the suite: 278 tests pass (23 files), build + lint clean. The long-carried
  "274" was WRONG and is corrected — see log #65's correction entry**
...
- **REGISTRY — THREE files now.** **TWENTY VERIFIED** (Michael) in `legal-rule-registry-discovery-and-carrier-duties.md`; five took **second observations** 08-12, and **three wording expansions there are FLAGGED, NOT ADOPTED** (TRCP 194's span, 193.3, 192.3(f)) — verification attaches to wording. **TWENTY-SEVEN UNVERIFIED** in `legal-rule-registry-discovery-enforcement-and-pleading.md` (discovery enforcement + UDJA/venue/pleading; **six WL/slip cites flagged for reporter-cite check, none corrected**). A new file, not a fold-in, because the verified file's status header would have broken — **PLACEMENT CONFIRMED by Michael 08-12; that flag is CLOSED.** **Count reconciled at HEAD 08-12 on commission: 27 entries = 22 deficiency + 5 UIM, one proposition each** (3 entries cite two cases, so 30 authorities). **SIX MORE UNVERIFIED 08-12** in `legal-rule-registry-criminal-plea-and-costs.md` (criminal classification/plea/costs) — **backlog 27 → 33**, and **two of those six carry NO CITE**: supplying a cite is a verification act, so it is YOURS. They **hard-gate CR-10**. **`/rules` SEED ALL UNVERIFIED**
- **NEW 08-13 (#65): `docs/specs/registry-cite-check-2026-08-13.md` — the cite-check memo, PROPOSED
  design input, research support only.** It stages evidence so each flagged check is **one look, not
  one hunt**: four of the six slip cites are memorandum opinions whose WL cite is PERMANENT (no
  upgrade exists), **Ochoa is the one where a reporter cite plausibly EXISTS ("(PUBLISH)")**, Collins
  needs retrieval; candidate cites are staged for both cite-less criminal entries. **It carries a
  FLAG worth your eye: art. 102.073's verbatim text cuts AGAINST criminal entry 4's "per cause"
  wording for single-criminal-action consolidated pleas** — likely a wording split before CR-10's
  cost check can consume it. **NOTHING is verified and NO registry file was touched** — every
  verification act stays yours. Queued as **V-2**
...
- **PRACTICE-PROJECT CHANNEL IS NOW THREE: CIVIL LIT + CRIM DEFENSE + PROBATE** (probate ruled 08-12; REQ-1 extended to each). New probate matters are CHATS inside PROBATE; **two grandfathered matter workspaces** (generic by design, never named build-side) reach the build side by ONE path: **PROBATE's client-clean REQ-CAPTUREs**. **Both the PROBATE creation and the CRIM DEFENSE paste-state were reported done 08-12 by you but are INVISIBLE to Code** — one word records both (**V-3**)
...
- **ALSO YOURS: re-upload SKILL.md v2** (#45 consolidated INTO SK-v2 per QR-1) · **the two sync-picker clicks**, `docs/skills/` (Q-3) and `docs/templates/` · **the `docs/reference/` exclusion click** · **V-1** (Descrybe ruling → tool routing only, or bump to v17 folding the two queued touch-ups?)
...
- **TELEMETRY RULED 08-12 (#63): OFFLINE LOCKDOWN.** `wandb`/`sentry-sdk`/OneLogger stay installed but are forced offline **machine-wide BEFORE any privileged audio is processed** — recipe at `docs/gpu-telemetry-offline.md`. **Your hand, on the GPU machine; closes when you report verification 3 passed.** **EMAIL-WORKFLOW DOC ADOPTED** (#63) at `docs/specs/email-workflow-requirements.md` — seven pipelines take **WF-2–WF-8**, every one gated (WF-5 doubly: money machinery unruled). **CL2-AC-1 is DIRECTION-CONFIRMED**
- **DESCRYBE IS OUT — RULED 08-13, until further notice** (revisit at go-live). FLP/CourtListener plus named public sources is the case-retrieval path. Whether this warrants an instructions revision is **V-1**, yours
...
- **`docs/reference/` HOLDS A BINARY — the first in `docs/`.** `CR3-code-sheet-2023.pdf` (Form CR-3CS 4/1/2023, blank public TxDOT code list; verified client-clean before it was committed). Synthesis at `docs/specs/cr3-field-code-map.md`, **PROPOSED input for IN-2/intake — nothing built.** Its load-bearing requirement: crash-report code tables are **VERSIONED external vocabularies**, so extraction must store **(code, decoded label, code-sheet version)**
- **VERIFICATION STATUS: the #62/#63 and #64 batches were VERIFIED design-side, FULL-TEXT, on 08-13 (#65)** — read from a connected read-only checkout at `f72de66`, not from RAG. **Their "design-unverified" flags are CLEAR.** Still UNREVIEWED: the carried **#31–#33** material and **#37–#61's own routing** — do not copy #36's clearance (log #37) forward. **#58 was VERIFIED design-side post-Sync, line by line**
- **Everything awaiting your ruling is in `docs/specs/attorney-review-queue.md`** — reconciled
  through **#65** (build entry #61 is Code's own), which added **V-1 · V-2 · V-3** with full question
  text. **K-6/K-7 RETIRED — reconstruct NOTHING.** **UNRULED, adopt nothing:
