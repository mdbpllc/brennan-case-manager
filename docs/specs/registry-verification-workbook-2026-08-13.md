...
> **SUPERSEDED IN PART — banner added 2026-08-17 (D-3, Michael's authorization at the Task 19 walk; `QR-6(e)`). ROWS BELOW ARE UNTOUCHED AND SOME OF THEM ARE STALE.** This workbook predates the V-5 split, the V-6/V-7 rewordings, the #95 wordings and the 2026-08-17 sign-off walk. **The enumeration of exactly how it is stale is `docs/specs/task-19-signoff-worklist-2026-08-17.md` §1**, and the current per-entry verification status is `docs/specs/task-19-signoff-record-2026-08-17.md` — read both before relying on any row here. **This file remains the numbering authority `V5-IDS` and `Q-T19-1` rest on**, which is why its rows were deliberately not rewritten. **FROZEN as the numbering record per Michael's ruling 2026-08-18 (WB2-1); the working layer for the open backlog is registry-verification-workbook-v2-2026-08.md.**
