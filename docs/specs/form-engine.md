...
2. Provider selection: checklist of linked FACILITIES (from the Medical tab's provider records — `R17`: the facility with its TYPE, and the individuals under it, each with a role marker; Michael selects facilities, the model names the individuals from the chronology, and the designee is always a PERSON, never the facility — FE-18, wording ratified 2026-08-31), each with any applicable flags surfaced beside the checkbox (see gates, §5; the ambient missing-information panel, `HD-1`; and the must-fix stops of REQ-CAPTURE §12.3).
3. Per-provider **interview card** (see §4) — pull-first, ask-only-if-missing. **RETIRED FOR THE TREATING TRACK 2026-08-31 (Michael's ruling, `AS-Q9`, `docs/specs/fe-d1-amendment-slice.md` §2):** under `RC-1` the writer draws treatment, future care and baseline facts from the chronology, and the clauses the cards filled (`{treatment_clause}`, `{future_care_clause}`, `{baseline_clause}`) no longer produce output; the credential guard survives as a writer instruction — never assert board certification unless the chronology or the record states it. Nothing is ruled about the cards outside the treating track; §4's text stands as the record of the retired mechanism.
...
7. Deadline display: the TRCP 195.2 designation rule STATED in-flow with its status — UNVERIFIED — and its inputs shown, as the FE-D1 build already does (spec-feedback 2026-08-20, finding 8); **NO date is computed, displayed or proposed until Michael verifies TRCP 195.2** (`R11` GATED; CLAUDE.md registry rule 1; the two register rows that carry the verification act are named at `REQ-CAPTURE_disclosures-expert-designation_2026-08-20.md` §18.E). *(Amended 2026-08-31 by Michael's ruling — `AS-Q12(a)`, `docs/specs/fe-d1-amendment-slice.md` §2. This step previously read: "Deadline display: computed 195.2 designation deadline shown in-flow (90 days before end of discovery period for parties seeking affirmative relief / 60 all others; DCO overrides when populated — matches deadline-engine rows E1/P11 incl. the known 90/60-vs-60/90 course-book conflict flag)." — text the FE-D1 build declined to implement under registry rule 1, spec-feedback 2026-08-20 finding 8.)*
...
- `{incident_date}`, `{incident_type}` (short phrase, e.g., "motor vehicle collision", set once per case in the wizard). **Event noun, RULED 2026-08-31:** where a FIXED sentence names the event (the EMT causation line; the writer is told the same rule for its opening), it is "collision" only for motor vehicle collisions and "incident" for everything else — Michael: "Let's make 'incident' the word for everything not a collision. I can always change those on my own if I want to later." The build derives the noun from the case's incident type; the per-case short phrase stays for prose.
- `{provider_name}`, `{credential}`, `{provider_dr_name}` (rendered name form, e.g., "Dr. Grimm"), `{provider_his_her}` / `{provider_their}` etc. — bare pronouns (render "his review", NOT "his's"); name-form tokens take `'s` in template text where possessive-by-name is wanted. Provider block (the person's name + credential suffix on top; the custodian line; facility name; address; phone) is uniform across variants and comes from the contact record. **CUSTODIAN LINE — RULED 2026-08-31 (`AS-Q7c`, `docs/specs/fe-d1-amendment-slice.md` §2; REQ-CAPTURE §11.4):** Part 3's written form **"And/or Custodian(s) of Records"**, the "(s)" following the count of named individuals; when nobody is named the block's TOP line is **"Custodian of Records"**; a pharmacy's is **"Pharmacist(s) and/or Custodian of Records"**. *(This spec carried "and/or Custodian of Records" until this edit; the literal at N ≥ 2 is a PROPOSED hands-on item, not settled.)* **WHICH FACILITY THE BLOCK READS — RULED 2026-08-31 (`AS-Q3`; slice §10 D-8; REQ-CAPTURE §17.1):** the block ALWAYS reads the **SELECTED** facility — the one the extracted name came out of — because a case-scoped provider record gives an unpromoted individual no affiliation edge at all. A promoted individual's `renders-care-at` edge, with its effective period selecting the facility current at the dates of treatment, is the world-fact form of the same fact; where the two DISAGREE the app raises a panel line and **never substitutes** (FE-18, ratified 2026-08-31; CD-14). **FE-18's ratified sentence speaks in the edge's terms and is NOT edited by this note — whether he re-ratifies it to the selection reading is PUT at REQ-CAPTURE §18.G.** ("Contact block" and "provider contact block" were this object's names in this spec until 2026-08-31; `HD-17` ruled the one name "provider block" and Michael ruled the rename applied at this edit.)
...
Shared skeleton: who they are → what they did for the client → testimony basis → testimony scope → damages elements → causation. Provider block above each paragraph comes from the contact record (it was called the "provider contact block" and the "provider identification paragraph" here until `HD-17` ruled the one name). Group versions = same template with plural tokens. **Since 2026-08-22 (widget B) these twelve are VOICE EXAMPLES for the writer, not output; since 2026-08-31 (`RC-1`) their BASIS and CAUSATION sentences are the SOURCE TEXT of the app's fixed-sentence table — the app places those two sentences, keyed by provider type, in singular or plural, and the writer composes the rest of the paragraph around them. See `REQ-CAPTURE_disclosures-expert-designation_2026-08-20.md` §11.6–§11.8.**
...
Notes: if a facility has both an EM physician and radiologists (e.g., the old Metropolitan Methodist block), the engine SPLITS them — EM paragraph for the treater, radiology paragraph for the readers, shared provider block. RULED 2026-08-31: the radiologist is the ONLY split at a mixed facility ("Radiologist is the only split"); every other type at the facility shares the facility's paragraph and its fixed sentences, and the radiologists are marked by Michael's hand on the Medical tab (REQ-CAPTURE §15.3, §17.1a).
...
Notes: **medical** probability — RULED 2026-08-31 (Michael: "Get rid of chiropractic probability and replace with medical probability"), superseding this note's earlier "chiropractic (not medical) probability — settled"; the sentence above carries the edit, and the chiropractor's causation sentence is therefore identical to the treating line (REQ-CAPTURE §11.8). No default future-treatment claims (add via per-provider notes when applicable). Most frequent LOP-gate variant.
...
- **Provider record (`R17`, minted 2026-08-31; CASE-SCOPED by `AS-Q3`, 2026-08-31 late):** the facility with its TYPE (set by Michael's hand, **per case, pre-filled from the last case where he set it**; the vocabulary is §9's PLUS the three added by `AS-Q5` — emergency medicine, pain management, orthopedic surgery, neurosurgery, primary care, chiropractic, physical therapy, prehospital EMS, radiologist, pharmacy, custodian-only, mid-level, **mental health**, **other physician (MD/DO)**, **other licensed non-physician provider** — **FIFTEEN values, not twelve**, REQ-CAPTURE §11.9); the individuals under it as **case-scoped rows, promoted to the firm-wide directory only by Michael's hand**, each with a ROLE MARKER defaulting to the facility's type (a radiologist is flipped by hand and takes a paragraph of its own); dates of treatment; one expandable summary per individual; sortable, oldest treatment first; `client_id` on the facility row (`AS-Q10` — provider list per client). **WHAT THE MODEL POPULATES — RULED 2026-08-31 late (`AS-Q2`, `docs/specs/fe-d1-amendment-slice.md` §2; REQ-CAPTURE §17.1a):** names, credential suffixes as the chronology shows them, dates of treatment, visit rows and a short per-individual "what they did" summary — **every field flagged model-extracted and UNVERIFIED** — and **NEVER the facility's type, an individual's role marker, or which facilities are in the case**, and **never a contact or a link**. *(This spec read "The model populates NAMES and nothing else" until this edit; that phrase is named a WORDING DEFECT by `AS-Q2` — it contradicted §17.7's auto-populated visit rows — and the mechanism is unchanged in substance.)*
- **The designee slot is typed (FE-18, ratified 2026-08-31):** an individual, or the person-role "Custodian of Records for {facility}" — never an organization; the charges table keys the FACILITY. The `renders-care-at` contact edge (CD-14, `contact-directory.md` §5) carries an effective period — **FROZEN into the `contact_edges` CHECK with `effective_from` / `effective_to` (date, nullable; a blank *to* means current) by `AS-Q11`, 2026-08-31 late; the edge type's name is no longer provisional.**
- **The fixed-sentence table (`RC-1`, 2026-08-31):** the BASIS and CAUSATION sentences of §9's variants, keyed by provider type, singular and plural, held as versioned data and generated from this spec with a drift test (as `variants.ts` already is); a paragraph is an ordered sequence of slots — writer opening, fixed basis, writer middle, fixed causation — and the writer's parts return as plain text.
