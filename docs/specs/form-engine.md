...
- `{provider_name}`, `{credential}`, `{provider_dr_name}` (rendered name form, e.g., "Dr. Grimm"), `{provider_his_her}` / `{provider_their}` etc. — bare pronouns (render "his review", NOT "his's"); name-form tokens take `'s` in template text where possessive-by-name is wanted. Provider block (the person's name + credential suffix on top; "and/or Custodian of Records"; facility name; address; phone) comes from the contact record — the facility at the far end of the person's `renders-care-at` link, with that link's effective period selecting the facility current at the dates of treatment (FE-18, ratified 2026-08-31; CD-14) — and is uniform across variants. ("Contact block" and "provider contact block" were this object's names in this spec until 2026-08-31; `HD-17` ruled the one name "provider block" and Michael ruled the rename applied at this edit.)
...
- **Provider record (`R17`, minted 2026-08-31):** the facility with its TYPE (set by Michael's hand; the type vocabulary is §9's — emergency medicine, pain management, orthopedic surgery, neurosurgery, primary care, chiropractic, physical therapy, prehospital EMS, radiologist, pharmacy, custodian-only, mid-level); the individuals under it, each with a ROLE MARKER defaulting to the facility's type (a radiologist is flipped by hand and takes a paragraph of its own); dates of treatment; one expandable summary per individual; sortable, oldest treatment first. The model populates NAMES and nothing else.
- **The designee slot is typed (FE-18, ratified 2026-08-31):** an individual, or the person-role "Custodian of Records for {facility}" — never an organization; the charges table keys the FACILITY. The `renders-care-at` contact edge (CD-14, `contact-directory.md` §5) carries an effective period.
