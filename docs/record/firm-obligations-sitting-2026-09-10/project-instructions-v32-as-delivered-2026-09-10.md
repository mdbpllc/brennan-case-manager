...
# v32 — 2026-09-10 (supersedes v31 of 2026-09-08. TRIGGER #3 FIRED in THE FIRM-OBLIGATIONS RULING SITTING
# (#155; typed, Cowork, Fable 5.1; device bridge on the checkout by STAGED read-only copies — the bridge VM's
# shell could not mount the folder, a new operational note below). ONE CONVENTION AMENDED: SOURCING gains a
# FIFTH and a SIXTH named channel by Michael's ruling (DECISION 9A, his pick verbatim "All five; (4) and (5) as
# named channels (Recommended)") — TIDC-published county indigent-defense plans, and the United States Code via
# the Office of the Law Revision Counsel / govinfo.gov — and the registry-discipline line gains the clause that
# the registry now covers the firm's PRACTICE as well as its cases (DECISION 8: a fifth registry file,
# `legal-rule-registry-firm-obligations.md`, drafted by a later Opus act; the cases-vs-practice boundary
# DECIDED by act). TRIGGERS #1, #2, #4, #5 and #6 DID NOT FIRE. THE SITTING ITSELF: eleven decisions and
# fifteen rendered-mock findings ruled one at a time on the firm-obligations sheet with the mock beside it
# (fourteen findings adopted as proposed; `FOM-12` — the weekend rule — as Michael's composite, re-asked twice
# narrower); `FO-5` and `FO-7` CLOSED; the `FO-`/`BR-`/`DA-` ids MINTED by his act (DECISION 10); the build
# slice `docs/specs/firm-obligations-build-slice.md` AUTHORIZED (`FOS-1`, his word "Yes"); the five spec §11
# hands-on items ACCEPTED onto the CC-1 queue as THE FIRM-OBLIGATIONS HANDS-ON SITTING; `Q-API-20` ruled
# "After". THE WORKING SET: THIRTEEN project docs at open (1,628,404 = 81.4%; the #154 pair had entered
# 2026-09-09); FIFTEEN after this sitting's capture/handoff pair enters as TRANSIT files — read as NOT a
# trigger-4 firing, the standing practice since v26 (the #151, #152 and #154 pairs entered the same way; the
# 2026-08-07 firing was an addition of RESIDENTS); the #137 Voice2 pair becomes TRANSIT-ELIGIBLE once batch 96
# is at HEAD, because this packet carries its `BR-2`/`BR-4`/`DA-2` definitions into `docs/record/` and mints
# its `FO-`/`BR-3` rows; its departure fires trigger #4 then. TWO OPERATIONAL NOTES ADDED (not rules): the
# device VM shell's mount failure and what it costs a sitting; the DECISION 0 widget slip. Nothing else changed
# from v31.)
...
- Capacity is measured in tokens against the context window, not megabytes;
  ...
  #155 pair's units are read at the next sitting.**) **THE UNIT IS EXACT (calibrated
...
4. The knowledge working set changes materially (sync selection changes,
   ...
   at HEAD, and its departure fires this trigger then.)
...
6. These instructions contradict repo state (BUILD-STATE, CLAUDE.md, or
   ...
   are overtaken by `#155`'s rulings, staleness for the runner's refresh.)
