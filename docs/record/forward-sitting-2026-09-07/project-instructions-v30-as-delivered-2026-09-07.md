...
# v30 — 2026-09-07 (supersedes v29 of 2026-09-02. TRIGGER #1 FIRED — THE GO-LIVE TRANSITION — and TRIGGER #4
# FIRED a fifth time, both in THE FORWARD SITTING (#149; typed, Cowork, Fable 5.1; device bridge on the checkout,
# `Documents\Knowledge Repo` and `Downloads`; Claude in Chrome for two acts). TRIGGER #3 DID NOT FIRE: the day's
# rulings are module design. TRIGGER #6 DID NOT FIRE. TRIGGER #5 is read as not firing (below).
# THE GO-LIVE FLOOR (GL-1) IS COMPLETE BUT FOR THE PASTE OF THIS TEXT. Michael chose to run the floor sitting
# inside the day's sitting (his pick "1"). Gate 9 CLOSED on evidence gathered by his own hand and ruled by him
# ("sure"): Custom SMTP on at `smtp.postmarkapp.com`, sender `signin@brennanstx.com`, read live from the Supabase
# dashboard; the 2026-08-20 round trip; Saturday 2026-09-05's magic links from that sender (a later-day repeat);
# and an OUTSIDE-mailbox round trip 2026-09-07 to a Proton Mail address he controls — by INVITE, because the sign-in
# box refused a non-user with "Signups are not allowed for the otp" (the signup toggle is OFF, the #125 finding
# closed) — Inbox, seconds, From `signin@brennanstx.com`, the temporary user deleted; the invite-for-magic-link
# substitution RULED TO COUNT, with one edge recorded (the magic-link TEMPLATE itself has only reached the firm's
# own tenant). The SMTP token lives only in Postmark's and Supabase's dashboards (his word). Gates 1 ("Still on
# Pro."), 2 (multi-user-only reading stands — "Yes"), 4 (no key exposure), 7/8 (billing analysis on real data
# deferred — "Confirmed") were confirmed ONE AT A TIME at his direction; 3, 5, 6, 10 confirmed at HEAD. The one
# LIVE matter is a TEST RECORD (his word: "test record") — no real client data has ever entered the database.
# The fix migration `2026-09-03-fe-d1-amendment-fix.sql` RAN BY HIS HAND during the sitting, all three checks
# answered in words — check 8 in both halves — so NO MIGRATION IS PENDING on the live database. GO-LIVE ITSELF
# remains GL-1's own words — real case, party, client and SOL data entered by hand into the core app, nothing
# more — and is Michael's act on his day; the two migrations the CCS-1 slice will write should run BEFORE it.
# ALSO THIS SITTING, all module design and none of it a convention: the FACILITY/PARTY ADDRESS MODEL ruled in five
# limbs (`spec-feedback.md` item 2/2a — "d": the case row picks the location, the registry stores street and
# city/state/ZIP as two fields; "formatted"; a legacy one-line address renders as two lines, split ONCE at the
# record by a stated rule and marked, THE RENDER PATH NEVER PARSES; one address shape instrument-wide; under the
# current tokens); the CC-1 RULINGS AND ADDRESS-MODEL SLICE authorized — `CCS-1` RULED YES ("yes", 17:11 CDT), put
# whole and then broken into seven limbs at his instruction, `HS-3` IN; `H12-v` moved — both external asks drafted
# ("1"); the NEXT MODULE is FIRM OBLIGATIONS ("Firm obligations first"); the CR-3 form and its versioned manual
# are in `Documents\Knowledge Repo` (Q-IN2-1 landed); `HS-8` deliberate; the leftover `inbox/` folder deleted by
# his hand; the seven remaining proposed hands-on items and the slice's eleven text acts go to THE POST-CCS-1 WALK.
# THE WORKING SET CHANGED (trigger #4): SIX captures/handoffs left project knowledge by the three-step TRANSIT act
# — the #147 pair, the #148 pair, and the #146 pair (found eligible at HEAD on the post-delete listing) — to a
# PERMANENT HOME Michael named and this session created: `Documents\Knowledge Repo\Session Captures\`. Meter:
# 1,582,076 = 79.1% at open → 1,559,064 = 78.0% → 1,547,818 = 77.4%; the unit calibration held to the unit a
# fourth and fifth time (23,012 and 11,246). NINE project docs remain (37,731 units = 1.9 points); the synced
# repo is ~75.5 points. The CC-1 hands-on queue stands at twelve accepted (all gated) plus seven proposed.
# ALSO CARRIED, operational notes and not rules: the magic-link / built-in-pane note (`HS-5`) with the signup-OFF
# and invite-substitution facts; the CRLF-after-checkout note (batch 88); cite `spec-feedback.md` by section date
# and heading, never by bare item number (four sections open with an item "2"). Nothing else changed from v29.)
...
- **SESSION CAPTURES ARE TRANSIT, NOT RESIDENTS (TC-8, ruled 2026-08-21 —
  ...
  Captures\`, created that day; the earlier relocation zips sit in
  `Downloads\` until his hand moves them).** They remain RAW
...
4. If information cannot be found in the repo, say so and suggest checking
   ...
   `Documents\Knowledge Repo\Session Captures\` (two zips there; the three
   09-02 zips and the 08-21 fifteen in `Downloads\`). Ask him rather than
...
- Capacity is measured in tokens against the context window, not megabytes;
  ...
  ~75.5 points.**) **THE UNIT IS EXACT (calibrated
...
- Canonical gates doc: docs/specs/Go_Live_Gates.md (pre-existing in the
  ...
  completes at the paste of this v30. Go-live is Michael's act on his day.**
...
1. Go-live transition — BEFORE the first real client record enters the
   ...
   SITTING (`#149`) and resolved in this v30**: the gates re-check was run to
...
6. These instructions contradict repo state (BUILD-STATE, CLAUDE.md, or
   ...
   instructions-vs-repo contradiction.)
