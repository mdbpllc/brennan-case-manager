...
# v33 — 2026-09-12 (supersedes v32 of 2026-09-10. TRIGGER #3 FIRED in THE FIRM-OBLIGATIONS HANDS-ON SITTING
# AND THE POST-`CCS-1` WALK (#156; typed, Cowork, Fable 5.1; device bridge on the checkout by STAGED read-only
# copies — the bridge VM's shell could not mount the folder, twice). ONE CONVENTION AMENDED: CAP-2 gains the
# CAP-2a clause by Michael's ruling (his pick verbatim "Edit rows carry — (Recommended)") — the class is a BIRTH
# rule for NEW files; a packet row that EDITS an existing canonical file in place inherits that file's class and
# carries `—`, which ends the open question the runner recorded from batch 89 through batch 96. TRIGGERS #1, #2,
# #4, #5 and #6 DID NOT FIRE. THE SITTING ITSELF: the firm-obligations module and the `CCS-1` address model
# walked by Michael in demo mode in his own Chrome, one question at a time — thirty-odd rulings, every one in his
# words in a running ledger; the §8-against-`FOD-4` collision RULED "(A)" (no code change); the Outlook reminder
# RULED as his composite (hard items only; 30 days by default; a per-obligation field); seven firm-obligations
# fixes and six disclosures fixes gathered into `docs/specs/firm-obligations-fix-slice.md`, AUTHORIZED whole
# (`FOS-2`, his word "Yes"); `FO-6` and per-paragraph regenerate ruled "later" (🟡); six `CCS-1` text acts left
# PROVISIONAL by name because he did not reach them. THE WORKING SET: FIFTEEN project docs at open (1,690,163 =
# 84.5%); SEVENTEEN after this sitting's capture/handoff pair enters as TRANSIT files — read as NOT a trigger-4
# firing, the standing practice since v26. ONE OPERATIONAL NOTE ADDED (not a rule): the demo app's port. Nothing
# else changed from v32.)
...
- **CAP-2 — EVIDENCE IS BORN UNSYNCED (ruled 2026-09-02).** Every packet routing
  ...
  **CAP-2a (ruled 2026-09-12, #156; Michael's pick verbatim "Edit rows carry —
  (Recommended)"): the class is a BIRTH rule for NEW files. A routing row that
  EDITS an existing canonical file in place — an append, an exact-match
  replacement, a register annotation — inherits that file's class and carries
  `—`; the runner checks the edit lands in the tree the file already lives in
  and records nothing further. This ends the open question batch 89's reading
  raised and batches 91–96 recorded on their faces.**
...
- SOURCING — STATUTES, RULES, AND REGULATIONS (ruled 2026-08-14, Q-STAT-1;
  ...
  - **County indigent-defense plans — the FIFTH NAMED CHANNEL (ruled
    2026-09-10, #155, DECISION 9A; Michael's pick verbatim: "All five; (4)
    and (5) as named channels (Recommended)"):** the indigent-defense plans
    the Texas Indigent Defense Commission publishes for each county, as
    PUBLIC documents saved to `Documents\Knowledge Repo\` by Michael's hand,
    cited by county, plan title and the plan's own adoption or effective
    date, each read named per item. Acquisition is his hand, permanently;
    WHICH counties' appointment lists he is on is HIS FACT (H5) and is never
    inferred from a listing. Unblocks the appointment-list eligibility row
    (`FOT-5`) of the firm-obligations catalog.
  - **The United States Code — the SIXTH NAMED CHANNEL (same ruling):** the
    Office of the Law Revision Counsel's official online publication
    (uscode.house.gov) and govinfo.gov, read per section, each read named
    per item with the section, the edition/supplement stated on the page
    and the request date — the federal STATUTE parallel to the eCFR
    channel for federal REGULATIONS (26 U.S.C. §§ 6072, 6654 and 6050I are
    the catalog's first uses). Claude cannot fetch binaries; a bulk title
    download, if ever wanted, is his hand. Retrieval is not verification.
...
- Registry discipline: every legal proposition any module relies on is a
  ...
  **The registry covers the firm's PRACTICE as well as its CASES (ruled
  2026-09-10, #155, DECISION 8 — "Registry entries, new file
  (Recommended)"): practice-of-law and practice-administration propositions
  — the firm-obligations catalog's first — live in a FIFTH file,
  `docs/specs/legal-rule-registry-firm-obligations.md`, drafted by a later
  Opus act on the FC-13 precedent with PF-1 running on that packet, each
  entry UNVERIFIED until Michael verifies it; the module's source notes point
  at the entries once they exist. The cases-vs-practice boundary BUILD-STATE
  carried as UNDECIDED since FC-13 is decided by that act.**
...
- Capacity is measured in tokens against the context window, not megabytes;
  ...
  design session can read WHEN Sync last ran instead of inferring it.
  Measured 2026-09-10 at THE FIRM-OBLIGATIONS RULING SITTING (#155):
  1,628,404 = 81.4% at open, THIRTEEN docs (the #154 pair having entered
  2026-09-09) — 17,709 above #154's packaging reading, consistent with a
  Sync after batch 95 plus the #154 pair's units, stated as consistent, not
  decomposed (no docs-API read this sitting); unchanged at packaging; the
  #155 pair's units are read at the next sitting. Measured 2026-09-12 at THE
  FIRM-OBLIGATIONS HANDS-ON SITTING (#156): 1,690,163 = 84.5% at open,
  FIFTEEN docs (the #155 pair having entered 2026-09-10) — 61,759 above
  #155's reading, consistent with a Sync after batch 96 plus the #155 pair's
  units, stated as consistent, not decomposed (no docs-API read this
  sitting).**) **THE UNIT IS EXACT (calibrated
...
- **THE DEVICE VM SHELL CAN FAIL TO MOUNT A GRANTED FOLDER, AND THE SITTING
  STILL RUNS (observed 2026-09-10, `#155`).** `device_request_folder_access`
  granted the checkout, but every `device_bash` call returned *"no Plan9
  drive shares mounted under /mnt/.virtiofs-root/shared"* and the folder
  never appeared under `$HOME/mnt/`; it stayed fully reachable by
  `device_list_dir`, `device_stage_files` and `device_commit_files`. The
  sitting read the checkout through STAGED read-only copies (the head file,
  BUILD-STATE, the live log, the registers, the specs it needed, `.git/`'s
  refs and reflog for HEAD), and delivered its packet by
  `device_commit_files` into `inbox/`. What the failure costs, stated so no
  session over-reads its own evidence: NO git command runs on his machine
  that sitting — so no `.git/index.lock` can be stranded, but also no live
  `ls-remote` is possible and the tracking ref is the only origin evidence
  (a local read, never a QR-3 pass); and NO on-device sha256 of a committed
  zip can be taken — the design side reports the zip's size and sha256 from
  the container, and the runner pins the identity at its Step 1 as always.
  Re-try the mount once; if it fails again, work the sitting this way rather
  than waiting.
- **A QUESTION SENT AS PROSE WITHOUT AN ANSWER WIDGET READS AS A STALL
  (observed 2026-09-10).** DECISION 0 went out as a message ending in a
  question; the turn ended with no widget, and Michael asked *"Are you
  running or did we get stuck here?"* In a typed ruling sitting every
  question is put through the answer widget so the turn visibly waits on
  him; prose introduces, the widget asks.
...
- **THE DEMO APP RUNS ON PORT 5175, NOT 5173 (read at `#156` from
  `package.json` at HEAD: `dev:demo` = `vite --mode demo --port 5175`).**
  README's `npm run dev` reaches SUPABASE mode on a machine whose `.env` is
  filled (`mdb-pllc` since 2026-08-20), so a hands-on sitting starts the demo
  with `npm run dev:demo` and opens `http://localhost:5175`. The Entra
  registration allows `localhost:5173` only, so a live Outlook check from the
  demo app needs `npx vite --mode demo --port 5173` — a command, never a
  tracked-file edit (the FO build's temporary `launch.json` change is not the
  pattern). Confirm both against `package.json` and BUILD-STATE at HEAD before
  giving the command; the README's tour predates `dev:demo`.
...
3. A binding convention is added, changed, or retired in a session — the
   ...
   **Fired TWICE MORE on 2026-08-18 — PF-1 from the fourth FABLE
   ...
   added in v31 is a note, not a rule; v31 is a trigger-4 edition.** **Fired
   ONCE on 2026-09-10 and resolved in this v32 — the SOURCING fifth and
   sixth channels (county indigent-defense plans; the U.S. Code via OLRC /
   govinfo.gov), with the registry-discipline practice clause riding the same
   edition — from THE FIRM-OBLIGATIONS RULING SITTING (`#155`, Fable 5.1,
   typed), which also ruled the module's eleven decisions and fifteen
   findings, authorized the slice (`FOS-1`), closed `FO-5` and `FO-7`, minted
   the `FO-`/`BR-3` rows, accepted the five hands-on items, and ruled
   `Q-API-20` "After".** **Fired ONCE on 2026-09-12 and resolved in this v33 —
   `CAP-2a`, the edit-row reading of the class column — from THE
   FIRM-OBLIGATIONS HANDS-ON SITTING AND THE POST-`CCS-1` WALK (`#156`, Fable
   5.1, typed), which also ruled the two walks' thirty-odd items, authorized
   the fix slice (`FOS-2`), and turned `FO-6` and per-paragraph regenerate 🟡.**)
4. The knowledge working set changes materially (sync selection changes,
   ...
   v31. DID NOT FIRE 2026-09-10: two capture files ENTERED (the `#155` pair)
   and none left; the `#137` Voice2 pair becomes eligible once batch 96 is
   at HEAD, and its departure fires this trigger then. DID NOT FIRE 2026-09-12:
   two capture files ENTERED (the `#156` pair) and none left; the `#137` Voice2
   pair's condition (2) is checkable at HEAD since batch 96 and was not
   verified — a later sitting with his Chrome.)
...
6. These instructions contradict repo state (BUILD-STATE, CLAUDE.md, or
   ...
   staleness for the runner's refresh, not contradiction. DID NOT FIRE
   2026-09-10: BUILD-STATE at `357ca8e` was read whole against this text and
   no contradiction found — its FIRM OBLIGATIONS bullet and NEXT ACTS line
   are overtaken by `#155`'s rulings, staleness for the runner's refresh. DID
   NOT FIRE 2026-09-12: BUILD-STATE at `e046906` was read whole against v32
   and no contradiction found — its FIRM OBLIGATIONS bullet, NEXT ACTS line
   and YOUR-HAND list are overtaken by `#156`'s acts, staleness for the
   runner's refresh.)
