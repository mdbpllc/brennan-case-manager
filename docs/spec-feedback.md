...
## 2026-09-17 — from the firm-obligations fix build (`FOS-2`): four stops ruled by Michael, the premises the slice carried that did not hold at HEAD, and the wording and design questions the build and its adversarial review surfaced

Recorded by the build session fired from `docs/prompts/PROMPT-firm-obligations-fix-slice-build-session.md` (Opus 5; queue runner BARRED), dated 2026-09-17 from the Central wall clock at its close (DT-1); the build ran 2026-09-16 into the small hours of 2026-09-17. **Nothing in any spec was edited.** Commits: `cc2413a` (group A), `5a98c92` (group B, the master `.docx` last), `5430bc0` (the whole-build review's fixes), and the close-out commit that carries this section. The session-log CODE entry of the same date carries the full record; this section carries only what needs the design side.

### A. Ruled by Michael at the build session's stops (2026-09-16 ~22:20 CDT, his picks among options Claude wrote, through the answer widget)

1. **The master `.docx` — the slice's own STOP (§3 item 16) fired.** The static heading `PLAINTIFF’S 194.2(b) & 195.5 DISCLOSURES` occurs exactly twice, but BOTH copies are in `word/document.xml`'s body — one under the caption, one heading the responses section after the certificate of service — and NEITHER is the footer. **Ruled: "Tokenize both headings (Recommended)"** — the first became `{instrument_title}`, the second `{footer_title}`. Evidence: `docs/record/firm-obligations-fix-build-2026-09-16/master-docx-byte-diff-2026-09-16.md`.
2. **Undo with no next.** §3 item 4's column test ("the closed occurrence's next exists … touched = false") would have refused Undo on a close that materialized no next — a one-time Done that retires its obligation (FOD-32) and a Done on a retired obligation — which FOS-1 allowed; and whether a close retired the obligation lives only in the close line's JSON. **Ruled: "Keep as built; read one flag (Recommended)"** — in his option's words Undo reads only the close record's `retiredObligation` flag; as built it also refuses where that record is absent (FXD-7, a reading); the columns decide everything else. §3 item 4's "no longer read by Undo" is superseded to that extent.
3. **The reminder days where no form sets them.** §3 item 9 and §7 item 8 said the demo fixture and the store upgrade carry 30 on EVERY row, against the composite's "30 days … or at the lead if the lead is shorter" (FOT-6, trust reconciliation, lead 5 and hard, would ring 25 days before it lights). **Ruled: "min(30, lead) everywhere (Recommended)"** — the fixture, the v18 upgrade and Add-as-inactive; the SQL column default stays 30.
4. **What sets `touched`.** §3 item 4's "the first edit, done, override or re-activation that reaches that next" would have ended Undo on a lead, weekend-rule, weight or reminder-days edit that only re-sends the event — against FOD-7 as built and ruled. **Ruled: "Same as today (Recommended)"** — a re-dating rule/precision edit, an override, or a close; the v18 upgrade sets it from the log by the same rule (FXD-7's "touched false" superseded).

### B. Premises the slice carried that did not hold at HEAD, and facts found building (not ruled; each built around as stated)

1. **The real footer is unreached.** `word/footer1.xml` carries DIFFERENT static text — `PLAINTIFF’S TRCP 194.2(B) AND 195.5 DISCLOSURES` — and the renderer substitutes only into `word/document.xml`; the Generate path refuses any other changed part, and a test pins the footers unchanged. So `R15`'s *"certificate of service and footer follow the title (FE-15)"* still does not reach the served FOOTER. Reaching it needs a renderer change and a change to that gate — a design act. (The unused seeded `footerTitle` value in `src/forms/seed.ts` matches the footer's text but for the apostrophe.)
2. **The two `R15` scalars did resolve before the build** — to `Plaintiff's TRCP 194.2(b) and 195.5 Disclosures` on one client — they reached no token. §1's "resolve nothing today" was about the document only. To keep a one-client instrument's two headings byte-identical, the scalars now resolve to the static heading's exact text (U+2019, `&`) on one client, while the filename, the generated-document record and the on-screen label keep the `R15` title as before.
3. **A multi-client heading now reads in the `R15` title's own form** — mixed case, "TRCP", "and", a straight apostrophe — at two bold, centred positions that print ALL CAPS with a typographic apostrophe on a one-client case (no `caps` run property exists to do it). As named by §3 item 16 and `R15`; **the appearance is Michael's question** (the unused `_caps` scalars exist; `(b)` would upper-case to `(B)`).
4. **Foot check 10's spelling.** §5.4 says `pg_get_constraintdef` "reads `is not distinct from`". PostgreSQL deparses that operator as `NOT (outcome IS DISTINCT FROM 'not-applicable'::text)`; a literal check would stop Michael on a correct database. Check 10 accepts either spelling and stops only on the old `=` form or no row. The review's SQL lane reasoned that PostgreSQL 17 prints `CHECK (((NOT (outcome IS DISTINCT FROM 'not-applicable'::text)) = (outcome_reason IS NOT NULL)))` — not run against a database.
5. **D-8's sentence was never rendered.** At HEAD `Currently practicing at …` existed only as a field on the designation block; nothing passed its input and the master had no token. Removing it (B1) changed no served document. **Panel line 17 never fires in the app** — nothing computes `affiliationMismatches` — and the two-edge fixture's covering edge names the selected facility, so §7 item 9's "still fires on the two-edge fixture" cannot hold; the build pins that line 17 fires when GIVEN a mismatch. Whether anything should compute one is a design question.
6. **`FXD-5`'s words are false at a pharmacy or custodian-only facility.** The marker pause and panel line 11 fire there too (their firing is unchanged, §3 item 14) and now say the person will be "designated in the treating paragraph once this pause is cleared", while D-46 designates nobody there and puts nobody on the block (line 15 says so beside it). Today's wording ("stay on the block") was false there as well. The pause's BODY sentence, which said the person is left out and drafted by hand, now carries `FXD-5`'s sentence (a reading).
7. **The seeded writer instructions still describe the retired clause.** Item 13 of `src/forms/writerInstructions.ts` (template version `ftv-writer-instructions-1`, riding every writer payload) still tells the writer what to return "if asked for a care-episode clause". The writer is never asked (B2). Removing it needs a new template version — not done.
8. **The reminder hint (`FXD-9`) can be false.** It shows whenever the reminder days exceed the lead. On a ROUTINE row nothing rings (`FXD-8`), and on a month-precision row (FOT-1, FOT-25) the row can light on the 1st of its month (FOM-6) before a reminder set above the lead rings. Built as `FXD-9` names it; the wording or condition is Michael's.
9. **"1 days".** The Details line reads "Outlook reminder 1 days before the target" at 1 — the same form as the existing "lead 1 days". PROVISIONAL wording.
10. **The bill form's placeholder "— (link provider party later)"** now points at a source that no longer feeds the picker (B6 reads the Providers section's list). Not reworded — a text act.
11. **The "Needs attention" sentence "a save to the central database did not finish"** names a cause that can no longer occur: every firm act is now one Postgres function, and the functions also re-check the plan's assumptions (C3 below). Only a hand edit in the database or a future defect could strand a row now. Not reworded — a text act.
12. **FOD-11's "weight never changes behaviour"** is no longer true on the Outlook limb: since `#156` A1 the weight decides whether the reminder rings. Register and card behaviour are unchanged. The code comments were corrected; the spec text is the design side's.
13. **Citation slips in the slice:** `R15` is in the `CCS-1` slice's **§2.1** (line 59), not §2.2; the static heading is spelled with U+2019 in the master, a straight apostrophe in the slice.
14. **No master path is named** in `docs/skills/drafting-disclosures/SKILL.md`; the app's master is `src/forms/skeletons/disclosures-plaintiff-v1.docx` through the base64 module `disclosuresSkeleton.ts` (no generator is committed; the build regenerated it by program and filed the method).
15. **Pharmacy blocks printed their literal twice too** before B4 (a pharmacy is always N = 0); the fix reaches them.
16. **The Forms tab's posture hint** — *"Drives the title, the certificate of service, and the footer name together (FE-15)."* — no longer describes a one-client instrument: its served heading is now the static heading whatever the posture (B2 above), while the filename, the record and the on-screen label still follow the posture. A PROVISIONAL text act, seen on the walk; not reworded.
17. **THIRD TRANCHE item 11's "empty" Garcia picker** did not reproduce by code at HEAD — a fresh store's picker offered the two linked provider parties. Unreconciled; moot after B6.

### C. Readings the build took that the slice does not make (built, reported, reversible)

1. The Inactive row's "Activate…" keeps the obligation's STORED reminder days when the lead is edited; only a NEW activation's field follows the lead until typed in. Otherwise a value raised by hand before retirement would be silently replaced (FXD-9). Two review lanes proposed following the lead there too, and both refuters refuted it.
2. A lead-only edit no longer re-sends the Outlook event — the event no longer depends on the lead. Weight and reminder-days edits do re-send it.
3. **Race guards in the nine functions (review finding).** The functions now lock the obligation first, and refuse when a concurrent act has changed what the plan assumed: a close whose plan opened no next because the obligation was retired, if it has since been re-activated; a re-activation whose plan opened nothing because an occurrence was open, if that occurrence has since closed; an Undo of a close with no next, if another occurrence has since been opened; and any call missing a required part. Each refusal is a PROVISIONAL "… reload the register." message.
4. **The database stamps `created_at` on every insert** (review finding): the order of closes, which Undo reads, is the server's clock in central mode, not the browser's. In demo mode it is the browser's clock, and a clock set backwards could misorder closes.
5. The Outlook-delete queue writes (`queueFirmOutlookDelete`, `settleFirmOutlookDelete`) are Outlook bookkeeping like the sync write: no review_log line, a plain update (read, compute, write) rather than a function. In central mode the write also bumps the obligation's `updated_at` through its trigger.
6. The bill picker lists each facility ONCE even where two clients share it, and is not scoped by client. It refreshes when the Providers section changes (review finding). The ledger's Provider column now falls back to the facility's name for a bill on an unlinked facility (review finding).
7. A closed occurrence's event carries `reminderMinutesBeforeStart: 0` beside `isReminderOn: false`, `FXD-8`'s shape for "off".

### D. For the design side — nothing here blocks the migration

- The migration is **amended and still not run**. Michael's hand runs it: a backup, STEP 0 alone with the result written down, the file pasted alone, and ten checks answered in words.
- Open by the record: the unreached footer (B1); the multi-client heading case (B3); panel line 17's missing source (B5); `FXD-5` at pharmacy/custodian-only (B6); the writer-instructions version (B7); the hint (B8); the placeholder, the stranded-row sentence and the posture hint (B10, B11, B16); FOD-11's spec wording (B12).
